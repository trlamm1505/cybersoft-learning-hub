...
def clean_orders_data(orders: pd.DataFrame) -> pd.DataFrame:
    ...
    cleaned["order_status"] = cleaned["order_status"].fillna("completed").replace("", "completed")
    return cleaned


def compute_kpis(orders: pd.DataFrame, order_items: pd.DataFrame, products: pd.DataFrame):
    ...
    merged = order_items.merge(orders[["order_id", "order_status"]], on="order_id", how="inner")
    merged = merged.merge(products[["product_id", "cost_price"]], on="product_id", how="inner")
    comp_items = merged[merged["order_status"] == "completed"].copy()

    comp_items["item_rev"] = (comp_items["unit_price"] - comp_items["discount_amount"]) * comp_items["quantity"]
    comp_items["item_cost"] = comp_items["cost_price"] * comp_items["quantity"]

    gross_profit = round(float((comp_items["item_rev"] - comp_items["item_cost"]).sum()), 2)
    gross_margin_pct = round((gross_profit / float(comp_items["item_rev"].sum())) * 100, 2)
...
def main():
    data_dir = os.path.join(os.path.dirname(__file__), "..", "..", "student_edition", "data")
