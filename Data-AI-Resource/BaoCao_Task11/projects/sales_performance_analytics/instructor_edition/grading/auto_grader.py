...
def grade_submission(submission_dir: Path, expected_kpis_path: Path) -> dict:
    ...
    if cleaned_file.exists():
        ...
        null_count = (
            df_clean["order_status"].isna().sum()
            + (df_clean["order_status"] == "").sum()
        )

        if (
            len(df_clean) == expected["total_clean_orders"]
            and dup_count == 0
            and null_count == 0
        ):
            ...
            results["feedback"].append(
                "CRIT-01 (16/20): Đã khử duplicate nhưng còn giá trị rỗng ở order_status."
            )
        else:
            results["scores"]["CRIT-01"] = 10.0
            results["feedback"].append(
                "CRIT-01 (10/20): Chưa khử triệt để bản ghi duplicate."
            )
    else:
        results["scores"]["CRIT-01"] = 0.0
        results["feedback"].append(
            "CRIT-01 FAIL (0/20): Không tìm thấy file orders_cleaned.csv."
        )
    ...
    if kpi_file.exists():
        ...
        if rev_err <= 0.05 and aov_err <= 0.1:
            ...
            results["feedback"].append(
                f"CRIT-02 PASS (25/25): Doanh thu ${sub_rev:,.2f} và AOV ${sub_aov:,.2f} khớp Ground Truth tuyệt đối."
            )
        elif rev_err <= 0.5:
            results["scores"]["CRIT-02"] = 20.0
            results["feedback"].append(
                f"CRIT-02 (20/25): Sai số doanh thu {rev_err:.2f}% (trong mức chấp nhận được)."
            )
        else:
            results["scores"]["CRIT-02"] = 12.0
            results["feedback"].append(
                f"CRIT-02 (12/25): Doanh thu sai số lớn {rev_err:.2f}%."
            )
    else:
        results["scores"]["CRIT-02"] = 0.0
        results["feedback"].append(
            "CRIT-02 FAIL (0/25): Không tìm thấy file kpi_summary.json."
        )
    ...
    if rfm_file.exists():
        ...
        if "segment" in df_rfm.columns and len(df_rfm["segment"].unique()) >= 4:
            ...
            results["feedback"].append(
                "CRIT-04 (10/15): Có phân khúc RFM nhưng chưa đủ 4-5 nhóm."
            )
    else:
        results["scores"]["CRIT-04"] = 0.0
        results["feedback"].append(
            "CRIT-04 (0/15): Chưa thực hiện bài toán mở rộng RFM."
        )
