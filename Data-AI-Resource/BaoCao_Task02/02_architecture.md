...
graph TB
    subgraph Users ["Người Dùng (Actors)"]
        MentorDA["Giảng viên Data Analyst"]
        MentorAI["Giảng viên AI Engineer"]
        Learner["Học viên DA & AI"]
        LP["CyberSoft Learning Platform"]
    end

    subgraph CyberSoft_Lab ["CyberSoft Data & AI Lab Core Engine"]
        API_Gateway["API Gateway / Router (FastAPI)"]

        subgraph Module1 ["1. Dataset Registry & Metadata"]
            CatalogMgr["Catalog Manager"]
            VersionControl["Data Versioning Engine"]
            MetaDB[(SQLite / DuckDB Metadata)]
        end

        subgraph Module2 ["2. Data Quality Harness"]
            SchemaValidator["Schema Validator (Pydantic)"]
            AnomalyDetector["7-Rule Quality Engine"]
            PIIScanner["PII & Privacy Scanner"]
        end

        subgraph Module3 ["3. Project Bank"]
            CapstoneMgr["Capstone Manager"]
            RubricEngine["Rubric 100-Point Grader"]
            SolutionVault["Solution & Ground Truth Vault"]
        end

        subgraph Module4 ["4. Semantic Search & RAG Tutor"]
            Chunker["Smart Markdown/PDF Chunker"]
            EmbeddingPipe["Embedding Pipeline (MiniLM)"]
            VectorStore[(ChromaDB Vector Store)]
            RAGTutor["Grounded RAG Tutor (Citation & Abstain)"]
        end

        subgraph Module5 ["5. Evaluation Harness"]
            GroundTruthBench["100-Q Ground Truth Benchmark"]
            MetricEvaluator["Eval Engine (Hit@K, MRR, Faithfulness)"]
            RegressionReport["Regression Test Reporter"]
        end
    end
    ...
    subgraph Storage ["Physical Storage"]
        ...
        DirtyDataDir["data/dirty/"]
        CorpusDir["corpus/documents/"]
    end

    MentorDA --> API_Gateway
    MentorAI --> API_Gateway
    Learner --> API_Gateway
    LP --> API_Gateway

    API_Gateway --> CatalogMgr
    API_Gateway --> AnomalyDetector
    API_Gateway --> CapstoneMgr
    API_Gateway --> RAGTutor
    API_Gateway --> MetricEvaluator

    CatalogMgr --> MetaDB
    CatalogMgr --> CleanDataDir
    CatalogMgr --> DirtyDataDir

    AnomalyDetector --> RawDataDir
    AnomalyDetector --> SchemaValidator
    AnomalyDetector --> PIIScanner

    RAGTutor --> VectorStore
    RAGTutor --> CorpusDir
    Chunker --> EmbeddingPipe --> VectorStore

    MetricEvaluator --> GroundTruthBench
    MetricEvaluator --> RAGTutor
```
