# Instructor Guide — LAB-08 Đối chiếu UI ↔ API khóa học

## Chuẩn bị

- Kiểm tra truy cập: https://demo2.cybersoft.edu.vn/
- Không phát `fixtures/manifest.json` hoặc tài liệu instructor cho học viên trước khi nộp.
- Reset fixture: `node fixtures/reset.mjs --mode buggy --lab LAB-08`.

## Đáp án định hướng

- Controlled fault: `F-D14-008`.
- Quy tắc đúng: Tổng UI và API của cùng bộ lọc/thời điểm phải khớp.
- Finding demo tham chiếu: Chưa có finding xác nhận; học viên tự khám phá.
...
1. Chạy `node fixtures/reset.mjs --mode clean --lab LAB-08`; fault phải biến mất.
